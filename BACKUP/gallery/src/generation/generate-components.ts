...
function generateHeaderComponent(nbGames: number, nbEvents: number): void {
	const headerMustache: string = fs
		.readFileSync("header.mustache")
		.toString();
...
function mapSnakeToPascalCase(str: string): string {
	...
	return split
		.map(
			(splitStr) =>
				splitStr.charAt(0).toUpperCase() + splitStr.substring(1),
		)
		.join("");
}
...
function getComponentNameFromEventDatabase(
	...
	return (
		mapSnakeToPascalCase(eventAlbum.title.replace(/ /g, "")) +
		eventAlbum.year
	);
}
	...
	const eventsComponents: string[] = [...eventAlbums]
		.sort((eventAlbum1, eventAlbum2) => {
			const component1: string =
				getComponentNameFromEventDatabase(eventAlbum1);
			const component2: string =
				getComponentNameFromEventDatabase(eventAlbum2);
			if (component1 === component2) {
		...
		.map((eventDatabase) =>
			getComponentNameFromEventDatabase(eventDatabase),
		);
